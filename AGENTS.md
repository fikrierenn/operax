# AGENTS.md — Operax WMS Platform

Bu dosya her Codex/Antigravity oturumunun başında okunur. **Değişmez kurallar + kimlik.** Detaylı kurallar, geçmiş kararlar ve süreç notları ilgili alt dosyalarda yaşar:

- `.Codex/rules/*.md` — davranış ve yazılım kuralları (yeni kod yazmadan önce mutlaka okunur)
...
- `plans/NN-<slug>.md` — Tier 3 işlerin plan dokümanları (plan-first kuralı — bkz. `.Codex/rules/plan-first.md`)
...
**Plan-First Disiplini:** 3+ dosya / yeni pattern / schema değişikliği / UI değişikliği gibi Tier 3 işler için **önce `plans/NN-<slug>.md` yazılır, kullanıcı onayı alınır, sonra kod yazılır**. Detay: [`.Codex/rules/plan-first.md`](file:///d:/Dev/Operax/.Codex/rules/plan-first.md).

**Faz Kapanış Kapısı (ZORUNLU):** Her Tier 3 fazı bitmeden: `build-validator` → `code-reviewer` → `sql-sp-reviewer` (SP varsa) → `security-reviewer` (yeni PageModel varsa) → manuel smoke (stok/ledger varsa). Detay: [`.Codex/rules/phase-review-gate.md`](file:///d:/Dev/Operax/.Codex/rules/phase-review-gate.md).
...
1. **Sistematik Çalış:** Konuşma geçmişine güvenme. Önemli kararlar ADR'ye, kurallar `.Codex/rules/` altına, süreç `TODO.md` ve `journal`'a yazılır.
...
4. **200 Satır Eşiği:** Bu dosya ve `.Codex/rules/*.md` dosyaları 200 satır altında kalmalıdır.
