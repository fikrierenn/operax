...
| B19 | **StockMovement.MovementDate (fiili hareket/kabul tarihi)** | Mikro sth_tarih≠create_date≠belge_tarih (MIKRO §14) + VUK 231 | Y | D | **OPERAX GAP — kritik.** StockMovement sadece CreatedAt (sistem anı); fiili hareket tarihi YOK. AccountMovement'ta MovementDate VAR (tutarsız). K4 dönem guard'ı MovementDate'e bağlanmalı (geç gelen belge doğru döneme). 3 tarih: belge(DocDate)≠işlenme(CreatedAt)≠fiili(MovementDate). → plan 14 BAĞIMLI · MIKRO §14 |
| B18 | **Hizmet/Masraf kalem tipi** (`Item.ItemKind`) | Mikro HIZMET_HESAPLARI/MASRAF_HESAPLARI ayrı master (MIKRO §13) | Y | O | **OPERAX GAP.** Hizmet (danışmanlık/nakliye) + masraf (kira/SGK) stoksuz kalem; Operax'ta hizmet/masraf kartı yok. `Item.ItemKind`=GOODS/SERVICE/EXPENSE discriminator + stoksuz kalemde StockMovement atlama guard'ı (AccountMovement+GL üretir, stok defteri kirlenmez). Masraf merkezi+dönemsel gider K1/ayrı modül. → MIKRO §13 |
