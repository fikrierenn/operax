...
- [ ] **🔴 PLATFORM · App-shell (sidebar) mobil-responsive DEĞİL (Plan 53 prototip tarayıcı-verify, 2026-06-23):** 375px'de sidebar (~240px) gizlenmiyor → içerik 135px'e sıkışıyor. `.doc-layout`/`.form-grid`/`.table-scroll` (yeni) içerik-katmanını doğru collapse ediyor (doğrulandı: tek-kolon + taşma yok) ama SHELL blocker. Gerekli: `_shell.css` + `_Layout` + JS — mobilde sidebar off-canvas/hamburger + overlay + topbar hamburger butonu. Gerçek mobil kullanım için ön-koşul. "Mobil sırası gelince" fazının ilk işi (kullanıcı: mobili sonra). Prototip (Expenses/Details) içerik-responsive hazır; shell eklenince tüm ekranlar mobil olur.
