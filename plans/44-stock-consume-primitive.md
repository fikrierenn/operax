...
- **Faz 3 ✅ BİTTİ 2026-06-22:** `sp_ConsumeInventoryFEFO` — bin belirtilmeyen çıkışta depo stoğunu FEFO (ItemLot.ExpiryDate, en yakın vade) → FIFO (en eski hareket) sırayla bin/lot'lara böler; her parça atomik `sp_ConsumeInventory`. item+depo applock (split atomik). Idempotency index BinId+LotNo eklendi (satır çok-bin'e bölünebilir). Material-issue: satır-bini varsa tek-bin, yoksa FEFO → IMP-2 (dağıtık-stok false-reject) ÇÖZÜLDÜ. Smoke: 4533 istek en-büyük-bin(4033)'ü aşınca **2 bin'e bölündü toplam tam** · oversell (depo toplamı) red. Shipping/transfer spesifik-bin → tek-bin kalır (doğru).
- **Faz 4 ✅ BİTTİ 2026-06-22:** `tr_StockMovement_Immutable` (AFTER UPDATE,DELETE) — fiziksel DELETE yasak (THROW 53010), immutable kolon UPDATE yasak (THROW 53011); yalnız iptal bayrağı (IsCancelled/CancelledAt/CancelledBy) güncellenebilir → reversal SP'leri geçer. 0-satır no-op (MovementDate backfill güvenli). Smoke 3/3: qty-update RED · delete RED · flag-update İZİNLİ. Audit HIGH (dönem guard sadece AFTER INSERT) kapandı. Harness cleanup DELETE→IsCancelled'a çevrildi (trigger uyumu).
