...
public class ProductionActivityService(Db db, ILogger<ProductionActivityService> logger)
{
    public async Task ConsumeMaterialAsync(Guid productionOrderId, Guid itemId, decimal qty, string? lotNo = null, string? serialNo = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            trans.Rollback();
            logger.LogError(ex, "Üretim hammadde sarfı hatası: {OrderId}", productionOrderId);
            throw;
        }
    }
    public async Task StartActivityAsync(Guid productionOrderId, Guid workCenterId, Guid routeStepId)
    {
        ...
        var active = await conn.QueryFirstOrDefaultAsync(@"
            -- DEAD/WIP kod — çoklu-firma izolasyonu burada uygulanmıyor çünkü bu servis hiçbir yerden
            -- çağrılmıyor (caller yok, DI kaydı yok) ve şemaya uymuyor (kırık StockMovement INSERT'leri).
            -- Gerçek üretim akışı SP kullanır (sp_ProductionFinish). Bu kod ileride yarım kalan WIP atölye
            -- terminali için SP'ye taşınarak yeniden yazılacak; o zamana kadar etkin değildir.
            -- isolation-guard:ignore  (operax-cli scan-isolation tarayıcısı bu işaretle sorguyu atlar)
            SELECT Id FROM ProductionActivity WHERE ProductionOrderId = @OrderId AND EndTime IS NULL", new { OrderId = productionOrderId });
