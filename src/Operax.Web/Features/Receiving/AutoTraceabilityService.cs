...
public class AutoTraceabilityService(Db db)
{
    ...
    public async Task<string> GenerateLotAsync(Guid itemId, Guid companyId, CancellationToken ct = default)
    {
    ...
    public async Task<string> GenerateSerialAsync(Guid itemId, Guid companyId, CancellationToken ct = default)
    {
        using var conn = db.Open();
        var item = await conn.QueryFirstOrDefaultAsync(
            new CommandDefinition(
    ...
    public async Task EnqueueLabelPrintAsync(string code, string type, string itemName, CancellationToken ct = default)
    {
