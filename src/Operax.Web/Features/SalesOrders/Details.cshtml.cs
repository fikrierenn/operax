...
public class DetailsModel(Db db, ICurrentCompany company, ICurrentUser user) : PageModel
{
    ...
    public async Task OnGetAsync(Guid? id)
    {
        ...
        if (id.HasValue)
        {
            Header = await conn.QueryFirstOrDefaultAsync<SalesOrderHeaderDto>(@"
                SELECT o.Id, o.WarehouseId, o.PartnerId, o.OrderNo, o.Status, o.Notes,
                       o.OrderDate, o.RequestedDeliveryDate, p.Name as CustomerName
    ...
    {
        // Satış siparişi başlığını kaydeder veya günceller (DRAFT)
        using var conn = db.Open();
    ...
    public async Task<IActionResult> OnPostAddLineAsync(Guid id, Guid itemId, decimal qty)
    {
        ...
        await conn.ExecuteAsync(
            "INSERT INTO SalesOrderLine (HeaderId, ItemId, UomId, QtyOrdered) VALUES (@HeaderId, @ItemId, @UomId, @Qty)",
