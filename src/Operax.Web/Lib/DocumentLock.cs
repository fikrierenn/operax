...
public static class DocumentLock
{
    ...
    public static async Task<bool> PoHasReceivingAsync(IDbConnection conn, Guid poId, Guid companyId)
        => await conn.ExecuteScalarAsync<int>(
            "SELECT COUNT(1) FROM ReceivingHeader WHERE PurchaseOrderId = @id AND CompanyId = @cid AND IsDeleted = 0",
            new { id = poId, cid = companyId }) > 0;
    ...
    public static async Task<bool> ReceivingHasInvoiceAsync(IDbConnection conn, Guid receivingId, Guid companyId)
        => await conn.ExecuteScalarAsync<int>(
            @"SELECT COUNT(1)
