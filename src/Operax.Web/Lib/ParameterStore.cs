...
public sealed class ParameterStore(Db db, ICurrentCompany company)
{
    ...
    public async Task<string> GetStringAsync(string code, string fallback, CancellationToken ct = default)
    {
    ...
    public async Task<int> GetIntAsync(string code, int fallback, CancellationToken ct = default)
    {
        var raw = await GetStringAsync(code, fallback.ToString(CultureInfo.InvariantCulture), ct);
    ...
    public async Task<decimal> GetDecimalAsync(string code, decimal fallback, CancellationToken ct = default)
    {
        var raw = await GetStringAsync(code, fallback.ToString(CultureInfo.InvariantCulture), ct);
